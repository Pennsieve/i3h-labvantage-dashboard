...
def convert_csv_to_parquet(config_path, data_dir, parquet_file):
    try:
        ...
        df, list_columns = apply_column_types(df, config)  # Get list columns
        
        # Write to Parquet with explicit schema for list columns
        print(f"\nWriting to Parquet file: {parquet_file}")
        
        # First convert to PyArrow table WITHOUT schema to let it infer types
    ...
    try:
        ...
        # Build PyArrow schema with explicit list types
        schema_fields = []
        for col in df.columns:
            if col in list_columns:
                # Create list<string> type for array columns
                schema_fields.append(pa.field(col, pa.list_(pa.string())))
            else:
                # Let PyArrow infer other types from pandas
        ...
        for col in list_columns:
            print(f"  {col}: {table.schema.field(col).type}")
