...
const loadStats = async () => {
  try {
    const [totalResult, studiesResult, participantsResult, typesCountResult] = await Promise.all([
      props.executeQuery('SELECT COUNT(*) as count FROM samples'),
      props.executeQuery('SELECT COUNT(DISTINCT STUDY) as count FROM samples'),
      props.executeQuery('SELECT COUNT(DISTINCT PARTICIPANTID) as count FROM samples'),
      props.executeQuery('SELECT COUNT(DISTINCT SAMPLETYPE) as count FROM samples')
    ])
